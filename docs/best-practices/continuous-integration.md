...
Microsoft Visual Studio Team Services (VSTS) and Visual Studio Team Foundation Server (TFS) use _fastlane_ in their [Apple App Store](https://marketplace.visualstudio.com/items?itemName=ms-vsclient.app-store) extension.
...
1. Go to your Visual Studio Team Services or TFS project, click on the Build tab, and create a new build definition (the "+" icon) that is hooked up to your project's appropriate source repo
...
The extension can be configured to install the latest version _fastlane_ or a specific version of _fastlane_:
