...
> The mercurial equivalent of the [commit_version_bump](https://docs.fastlane.tools/generated/actions/commit_version_bump/) git action. Like the git version, it is useful in conjunction with [`increment_build_number`](https://docs.fastlane.tools/generated/actions/increment_build_number/).<br>It checks the repo to make sure that only the relevant files have changed, these are the files that `increment_build_number` (`agvtool`) touches:
