...
To get a list of all available options see the [`hockey` action docs](https://docs.fastlane.tools/generated/actions/hockey/), or run 
...
This isn't recommended, however some teams prefer this approach. You can use the number of commits of the current branch (via [`number_of_commits`](https://docs.fastlane.tools/generated/actions/number_of_commits/)) as the build number. This will only work if you always run the build on the same branch.
